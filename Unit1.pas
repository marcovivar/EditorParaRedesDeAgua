...
begin
  if saveTextFileDialog1.Execute() then
    begin
      ...
      for i := 1 to nTubo do
        begin
          writeLn(fp,tubo[i].x1, ' ', tubo[i].y1, ' ', tubo[i].x2, ' ', tubo[i].y2);
        end;
      for i := 1 to nManguera do
        begin
          writeLn(fp,manguera[i].x1, ' ', manguera[i].y1, ' ', manguera[i].x2, ' ', manguera[i].y2);
        end;
...
begin
  if openTextFileDialog1.Execute() then
    ...
    //pintamos lo que se cargo del archivo
